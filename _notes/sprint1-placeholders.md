...
- **Featured Markets grid — 10 of 12 markets now have photos.** ✅ Resolved 2026-04-26 by sourcing CC-licensed editorial photography from Wikimedia Commons (Larry D. Moore + others), Pexels, and Unsplash — 47 candidates researched, 10 wired into the homepage cards (1 strongest landmark per market). All 10 use `<picture>` with webp + jpg fallback, eager-load on the first 4 cards, lazy on the rest. Per-card credit caption + `/credits/` page satisfies CC-BY/BY-SA attribution. Source-of-truth manifest at `/images/markets-research/MANIFEST.json`. **Two markets still need landmark shots:** Kyle 78640 (Old Town Kyle / Plum Creek / Lake Kyle) and South Austin 78745 (South Lamar / Garrison Park / Manchaca). Both ship a "Coming soon" treatment until commissioned. Original Kilpatrick FB/IG archive (~738 photos) was ~95% Canva composites with brand overlay and didn't work as editorial cards — superseded by the licensed-photography approach.
...
- Featured Markets grid on homepage links the 4 Tier 1 cards (Belterra/Bear Creek 78737, South Austin 78745, Dripping Springs, SW Austin 78749) to their dedicated guides; the remaining 8 cards still link to `/neighborhoods/` until those Sprint 2 guides ship. Update each card's `href` when its dedicated guide lands.
